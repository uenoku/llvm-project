...
; CHECK-SAME: (i32 [[TMP0:%.*]])
; CHECK-NEXT:    [[TMP2:%.*]] = tail call i32 @r1(i32 undef)
; CHECK-NEXT:    br label [[TMP4:%.*]]
; CHECK:       3:
; CHECK-NEXT:    unreachable
; CHECK:       4:
...
; CHECK-SAME: (i32 [[U:%.*]])
; CHECK-NEXT:  entry:
; FIXME: range [0, inf] should be here
...
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi i32 [ [[ADD]], [[IF_THEN]] ], [ 0, [[IF_ELSE]] ]
; CHECK-NEXT:    ret i32 [[RETVAL_0]]
;
entry:
  %cmp = icmp sgt i32 %u, -1
...
return:                                           ; preds = %if.else, %if.then
  %retval.0 = phi i32 [ %addneko, %if.then ], [ 0, %if.else ]
  ret i32 %retval.0
}
