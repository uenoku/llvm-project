...
public:
  ...
  bool isWithinMultithread() const { return true; }
...
public:
  DialectReader(AttrTypeReader &attrTypeReader,
                ...
                const ResourceSectionReader &resourceReader,
                const llvm::StringMap<BytecodeDialect *> &dialectsMap,
                EncodingReader &reader, const uint64_t &bytecodeVersion)
...
private:
  ...
  const llvm::StringMap<BytecodeDialect *> &dialectsMap;
  EncodingReader &reader;
  const uint64_t &bytecodeVersion;
};
...
private:
  ...
  SmallVector<std::unique_ptr<IsolatedRegionReader>> regionReaders;
...
BytecodeReader::Impl::parseIRSection(ArrayRef<uint8_t> sectionData,
  ...
  regionReaders.push_back(std::make_unique<IsolatedRegionReader>(this));
  IsolatedRegionReader &regionReader = *regionReaders.back().get();
  ...
  while (!regionStack.empty())
    if (failed(regionReader.parseRegions(regionStack, regionStack.back(), true,
                                         isolatedRegions)))
      return failure();
...
FailureOr<Operation *>
BytecodeReader::Impl::IsolatedRegionReader::parseOpWithoutRegions(
  ...
  if (opMask & bytecode::OpEncodingMask::kHasProperties) {
    ...
    if (!wasRegistered)
      return emitError(parentReader->fileLoc,
                       ...
                       "bytecode version ")
             << parentReader->version << " with properties.";
